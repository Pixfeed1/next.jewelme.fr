...
import { cmsUrl, idLangFromLocale } from '@/lib/url-builder';
...
export default async function Footer() {
  ...
  // URLs CMS natives Presta résolues côté serveur (fallback cmsUrl).
  const cmsUrlMap = cms_pages.length > 0
    ? await resolveUrls(cms_pages.map((p) => ({ type: 'cms', id: p.id })), idLangFromLocale(locale))
          ...
          {cms_pages.length > 0 && (
            <div>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
                {cms_pages.map((page) => (
                  <li key={page.id}>
                    <a href={cmsUrlMap.get(`cms:${page.id}`) ?? cmsUrl({ id: page.id, slug: page.link_rewrite }, locale)}
