...
import { parseIdFromSlug, categoryUrl, productUrl, brandUrl } from '@/lib/urls';
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string;  id: string }> }) {
  ...
  const productId = parseIdFromSlug(id);
  if (productId === 0) return {};
  const product = await fetchProduct(productId);
  if (!product) return {};
  const url = productUrl(locale, product.id, product.linkRewrite);
  const metaTitle = product.metaTitle || product.name;
  const metaDescription = product.metaDescription || product.descriptionShort || '';
  const urlFr = productUrl('fr', product.id, product.linkRewrite);
  const urlEn = productUrl('en', product.id, product.linkRewrite);
  return {
    title: metaTitle,
...
export default async function ProductPage({
  ...
  const category = product.idCategoryDefault > 0
    ? await fetchCategory(product.idCategoryDefault).catch(() => null)
    : null;
      ...
      <p style={{ marginBottom: 24, fontSize: 13 }}>
        ...
        {category && (
          <>
            ...
            <Link href={categoryUrl(locale, category.id, category.linkRewrite)} style={{ color: '#888', textDecoration: 'none' }}>
              {category.name}
            </Link>
          </>
        )}
      ...
      <div style={{ padding: '32px', border: '1px solid #e5e0d6', borderRadius: 4, background: '#fff', marginBottom: 32 }}>
        ...
        <div>
          ...
          {product.manufacturerName && (
            <p style={{ color: '#666', fontSize: 14, marginBottom: 8, marginTop: 0 }}>
              Label : {product.idManufacturer > 0 ? (
                <Link href={brandUrl(locale, product.idManufacturer)} style={{ color: 'var(--or-text)', fontWeight: 500, textDecoration: 'none', borderBottom: '1px solid currentColor' }}>{product.manufacturerName}</Link>
              ) : (
          ...
          <ProductSamplePlaylist
            productId={product.id}
            productName={product.name}
            productLink={productUrl(locale, product.id, product.linkRewrite)}
