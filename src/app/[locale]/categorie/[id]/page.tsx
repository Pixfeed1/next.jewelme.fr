...
import { parseIdFromSlug, categoryUrl } from '@/lib/urls';
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string;  id: string }> }) {
  ...
  const idCat = parseIdFromSlug(id);
  if (idCat === 0) return {};
  const idLang = await getServerIdLang();
  const t = await getServerT();
  const category = await fetchCategory(idCat, idLang);
  if (!category) return {};
  const url = categoryUrl(locale, category.id, category.linkRewrite);
  const metaTitle = category.metaTitle || category.name;
  const metaDescription = category.metaDescription || '';
  const urlFr = categoryUrl('fr', category.id, category.linkRewrite);
  const urlEn = categoryUrl('en', category.id, category.linkRewrite);
  return {
    title: metaTitle,
    ...
    <div>
      ...
      {totalPages > 1 && (
        <nav style={{ display: 'flex', justifyContent: 'center', gap: 16, marginTop: 48, alignItems: 'center' }}>
          {page > 1 ? (
            <Link href={`${categoryUrl(locale, idCat, category.linkRewrite)}?page=${page - 1}`} style={{ padding: '8px 16px', border: '1px solid var(--or-grey-lighter)', borderRadius: 4, textDecoration: 'none', color: 'var(--or-text)' }}>
              ← Précédent
            </Link>
          ) : null}
