  ...
  if (!cart || cart.items.length === 0) {
    return (
      <div style={{ maxWidth: 600, margin: '0 auto', padding: 48, textAlign: 'center' }}>
        ...
        <Link href={`/${locale}`} style={{ display: 'inline-block', marginTop: 16, padding: '12px 28px', background: '#a3a2a2', color: '#fff', textDecoration: 'none', borderRadius: 4, fontSize: 13, fontWeight: 600, textTransform: 'uppercase' }}>Retour boutique</Link>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 16px' }}>
      <p style={{ marginBottom: 20, fontSize: 13, color: '#888' }}>
        ...
        <Link href={`/${locale}/panier`} style={{ color: '#888', textDecoration: 'none' }}>Panier</Link>
        <span style={{ margin: '0 8px' }}>›</span>
      ...
      <div style={{ marginBottom: 20 }}>
        <Link href={`/${locale}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: '#333', color: '#fff', textDecoration: 'none', fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.05em', borderRadius: 3 }}>
          ...
          {step === 4 && init && (
            <section style={sectionStyle}>
              ...
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginTop: 24, fontSize: 13, cursor: 'pointer' }}>
                ...
                <span>J&apos;accepte les{' '}
                  {init.cgv ? (
                    <Link href={`/${locale}/page/${init.cgv.slug}`} target="_blank" style={{ color: '#666', textDecoration: 'underline' }}>{init.cgv.title}</Link>
