import { NextRequest, NextResponse } from 'next/server';
import { cookies } from 'next/headers';
import { postController, AUTH_COOKIE } from '@/lib/auth-server';

// Avoir PDF d'une commande (le backend verifie l'appartenance) -> application/pdf.
export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const token = String(body.token || '');
    const id_order = body.id_order;
    const customerToken = (await cookies()).get(AUTH_COOKIE)?.value || '';
    if ((!token && !customerToken) || !id_order) {
      return NextResponse.json({ success: false, error: 'missing params' }, { status: 400 });
    }
    const r = await postController('customer_credit_slip_pdf', { token, customer_token: customerToken, id_order });
    const contentType = r.headers.get('content-type') || '';
    if (!r.ok || !contentType.includes('pdf')) {
      const txt = await r.text();
      return new NextResponse(txt, { status: r.ok ? 502 : r.status, headers: { 'Content-Type': contentType || 'application/json' } });
    }
    const buf = await r.arrayBuffer();
    return new NextResponse(buf, {
      status: 200,
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="avoir-${id_order}.pdf"`,
      },
    });
  } catch (e: unknown) {
    return NextResponse.json({ success: false, error: e instanceof Error ? e.message : String(e) }, { status: 500 });
  }
}
